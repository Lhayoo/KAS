...
class HomeModel extends Database
{
    public function getInfo()
    {
        ...
        $info['kas'] = $this->connect->query("SELECT `kas`.`jumlah`,`kas`.`users_id`,`kas`.`tanggal`,`kas`.`status`,`warga`.`nama` FROM kas,warga,users WHERE `kas`.`status`='belum' AND `kas`.`users_id`=`users`.`id` AND `users`.`NIK`=`warga`.`NIK` AND `kas`.`tanggal` BETWEEN '$tgl1' AND '$tgl31' ORDER BY `kas`.`tanggal` DESC");
        return $info;
    }
}