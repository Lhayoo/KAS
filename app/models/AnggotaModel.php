...
class AnggotaModel extends Database
{
    public function getInfo()
    {
        ...
        $info['total_kas'] = $this->connect->query("SELECT SUM(jumlah) AS total FROM kas WHERE id = '$id' AND `status` = 'bayar'")->fetch_assoc()['total'];
        ...
        $info['pemasukan'] = $this->connect->query("SELECT * FROM pemasukan WHERE `pemasukan`.`tanggal` BETWEEN '$tgl1' AND '$tgl31' ORDER BY `pemasukan`.`tanggal` DESC")->fetch_all(MYSQLI_ASSOC);
        $info['pengeluaran'] = $this->connect->query("SELECT * FROM pengeluaran WHERE tanggal BETWEEN '$tgl1' AND '$tgl31' ORDER BY tanggal DESC");
        $info['kas'] = $this->connect->query("SELECT * FROM kas WHERE id = '$id' ORDER BY tanggal DESC");
        return $info;
    }
}